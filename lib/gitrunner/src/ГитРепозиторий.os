...
// Вывести историю репозитория git log
//
// Параметры:
//   Графически - Булево - Вывести историю в виде графа (--graph)
//   ПроизвольнаяКоманда = Строка - Дополненая командвы вывода истории
Процедура ВывестиИсторию(Графически = Ложь, ПроизвольнаяКоманда = "") Экспорт

	ПараметрыЗапуска = Новый Массив;
	Если ЗначениеЗаполнено(ПроизвольнаяКоманда) Тогда
		ПараметрыЗапуска.Добавить(ПроизвольнаяКоманда);
	Иначе
...
// Выполнить произвольную команду git
//
// Параметры:
//   ПроизвольнаяКоманда - Строка - текст команду 
Процедура ВыполнитьПроизвольнуюКомандуГит(ПроизвольнаяКоманда) Экспорт
